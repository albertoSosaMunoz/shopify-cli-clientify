...
export class ClientifyService {
  ...
  async getAccountInfo(): Promise<ClientifyAccountInfo> {
    ...
    logger.info(`✅ Cuenta: ${accountInfo.name} (${accountInfo.email})`);
    return accountInfo;
  }
  ...
  async syncContact(contactData: ClientifyContact): Promise<number> {
    const shopifyId = contactData.custom_fields?.find(f => f.field === "shopify_customer_id")?.value;
    // 1. Buscar por Shopify ID
    let existingContact = shopifyId ? await this.findContactByShopifyId(shopifyId) : null;
    ...
    if (existingContact) {
      logger.info(`✅ Contacto encontrado en Clientify (ID: ${existingContact.id}), actualizando...`);
      ...
      logger.info(`✅ Contacto creado con ID: ${newContact.id}`);
  ...
  async syncProduct(productData: ClientifyProduct): Promise<number> {
    const shopifyVariantId = productData.custom_fields?.find(f => f.field === "shopify_variant_id")?.value;
    
    // 1. Buscar por Shopify Variant ID
    let existingProduct = shopifyVariantId ? await this.findProductByShopifyId(shopifyVariantId) : null;
    ...
    if (existingProduct) {
      logger.info(`✅ Producto encontrado en Clientify (ID: ${existingProduct.id}), actualizando...`);
      
      // Crear copia de productData sin SKU para actualización (owner sí se incluye)
  ...
  async syncDeal(dealData: ClientifyDeal): Promise<number> {
    const shopifyOrderId = dealData.custom_fields?.find(f => f.field === "shopify_order_id")?.value;
    
    // 1. Buscar por Shopify Order ID
    let existingDeal = shopifyOrderId ? await this.findDealByShopifyOrderId(shopifyOrderId) : null;

    // 2. Actualizar o crear
    if (existingDeal) {
      logger.info(`✅ Deal encontrado en Clientify (ID: ${existingDeal.id}), actualizando...`);
      ...
      logger.info(`✅ Deal creado con ID: ${newDeal.id}`);
      return newDeal.id;
    }
  }
  ...
  async createDeal(dealData: ClientifyDeal): Promise<number> {
    logger.info(`📦 Creando oportunidad en Clientify...`);
    const newDeal = await this.request(`/deals/`, {
      method: "POST",
      body: JSON.stringify(dealData),
    });
    logger.info(`✅ Oportunidad creada con ID: ${newDeal.id}`);
    return newDeal.id;
  }
}
